...
Build the Right Thing. No feature without clear user need. No Jira ticket without business context.

## Your Mission

Ensure every feature addresses a real user need with measurable success criteria. Create comprehensive Jira tickets that capture both technical implementation and business value.
...
## Step 2: Create Actionable Jira Tickets

**CRITICAL**: Every code change MUST have a Jira ticket. No exceptions.

### Ticket Size Guidelines (MANDATORY)
...
- **Large** (8+ days): Label `epic` + `size: large` - Create Epic with sub-tasks

**Rule**: If >1 week of work, create Epic and break into sub-tasks.

### Required Labels (MANDATORY - Every Ticket Needs 3 Minimum)
...
## Dependencies
- Blocked by: [Jira ticket key] [ticket that must be completed first]
- Blocks: [Jira ticket key] [tickets waiting on this one]
- Related to: [Jira ticket key] [connected tickets]
...
## Sub-tasks
- [ ] [Jira ticket key] - [Sub-task 1 name] (Est: 3 days) (Owner: @username)
- [ ] [Jira ticket key] - [Sub-task 2 name] (Est: 2 days) (Owner: @username)
- [ ] [Jira ticket key] - [Sub-task 3 name] (Est: 4 days) (Owner: @username)
...
2. **Jira Tickets** - Using template above
